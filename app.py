...
CATEGORY_COLORS = {
    ...
    "undefined": ("#94A3B8", "rgba(148, 163, 184, 0.15)")
}
...
<style>
    ...
    @keyframes radar-glow {
        0%, 100% { filter: brightness(1) drop-shadow(0 0 0px rgba(0, 230, 118, 0)); }
        50% { filter: brightness(1.4) drop-shadow(0 0 5px rgba(0, 230, 118, 0.8)); }
    }

    .live-badge {
        display: inline-flex; align-items: center; background-color: var(--surface);
        padding: 6px 12px; border-radius: 20px; border: 1px solid var(--border);
        font-size: 13px; font-weight: 600; margin-bottom: 15px; gap: 6px;
    ...
    .radar-icon { animation: radar-glow 2s infinite ease-in-out; }
    .live-count { color: var(--success); font-weight: 700; }
    .live-text { color: var(--muted-foreground); }
    ...
    .event-card {
        background-color: var(--card); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
        border-radius: 0.75rem; padding: 1.25rem; margin-bottom: 24px; cursor: pointer;
        animation: fade-in 0.3s ease-out forwards; transition: all 0.3s ease; 
        border: 1px solid var(--border); height: 310px; display: flex; flex-direction: column;
        font-family: 'Inter', sans-serif;
    }
    .event-card:hover {
    ...
    .pill-category { font-size: 0.75rem; font-weight: 600; padding: 0.25rem 0.625rem; border-radius: 9999px; border: 1px solid transparent; }
    .pill-deal { font-size: 0.75rem; font-weight: 600; padding: 0.25rem 0.625rem; border-radius: 9999px; 
    ...
    .card-title { font-weight: 700; color: var(--foreground); font-size: 1.1rem; margin-bottom: 0.75rem; line-height: 1.375; 
                  transition: color 0.2s; margin-top: 0; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; 
    ...
    .btn-primary { display: inline-flex; align-items: center; padding: 0.5rem 1rem; font-size: 0.8rem; font-weight: 700; 
    ...
    .btn-primary:hover { box-shadow: 0 0 20px rgba(0, 210, 255, 0.6); transform: scale(1.02); background-color: #33DBFF; }
    
    .btn-secondary { display: inline-flex; align-items: center; padding: 0.5rem 1rem; font-size: 0.8rem; font-weight: 700; 
    ...
    .btn-secondary:hover { background-color: rgba(0, 210, 255, 0.2); box-shadow: 0 0 15px rgba(0, 210, 255, 0.3); transform: scale(1.02); }
...
if not df.empty:
    ...
    # --- NO-RELOAD FILTER BAR IMPLEMENTATION ---
    st.write("") 
    ...
    with filter_container:
        ...
        with f_pill_col:
            # 1. Sort categories so indices are consistent for CSS mapping
            all_categories = sorted(list(df['category'].dropna().unique()))
            filter_options = ["All"] + all_categories
            
            # 2. Get current category from Streamlit Query Params securely
            initial_cat = st.query_params.get("category", "All")
            default_idx = filter_options.index(initial_cat) if initial_cat in filter_options else 0

            # 3. Callback to update URL seamlessly when clicked
            def on_category_change():
                st.query_params["category"] = st.session_state.category_radio

            # 4. Render Native Streamlit Radio Widget
            ...
            # 5. Inject Dynamic CSS to magically style the Radio Widget into Glass Pills
            radio_css = """<style>
            /* Make the background a glass container */
            [data-testid="stRadio"] {
                background-color: var(--surface); backdrop-filter: blur(10px);
                padding: 12px 16px; border-radius: 0.75rem; border: 1px solid var(--border);
            }
            /* Make the 'Filter by' text neon and uppercase */
            [data-testid="stRadio"] > label {
                font-size: 0.8rem !important; font-weight: 700 !important; color: var(--foreground) !important;
                text-transform: uppercase !important; letter-spacing: 0.5px !important; margin-bottom: 8px !important;
            }
            /* Hide the actual radio circle selectors */
            [data-testid="stRadio"] div[role="radiogroup"] > label > div:first-child { display: none; }
            /* Align them horizontally with a gap */
            [data-testid="stRadio"] div[role="radiogroup"] { display: flex; gap: 10px; flex-wrap: wrap; }
            /* Base shape of the pill */
            [data-testid="stRadio"] div[role="radiogroup"] > label {
                padding: 8px 16px !important; border-radius: 9999px !important; font-size: 0.85rem !important;
            ...
            [data-testid="stRadio"] div[role="radiogroup"] > label p { color: inherit !important; font-weight: 600 !important; }
            """

            # Loop through options and assign specific colors using CSS nth-child
            for i, cat in enumerate(filter_options, 1):
                if cat == "All":
                    color, bg = ("#F8FAFC", "rgba(248, 250, 252, 0.15)")
                else:
    ...
    # Apply filters based on the selected Radio value
    filtered_df = df.copy()
    if selected_cat != "All":
    ...
    with tab1:
        ...
        for index, row in filtered_df.reset_index().iterrows():
            ...
            deal_html = f'<p class="deal-text" title="{deal_note}">{deal_note}</p>' if deal_note else ''
