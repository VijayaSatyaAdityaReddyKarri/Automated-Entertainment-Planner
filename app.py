...
# Robust color lookup to prevent missing colors due to DB string variations
def get_category_style(cat_name):
    cat_str = str(cat_name).strip().lower()
    for key, val in CATEGORY_COLORS.items():
        if key.lower() == cat_str:
            return val
    return CATEGORY_COLORS["undefined"]
...
<style>
    ...
    .live-badge {
        display: inline-flex; align-items: center; background-color: var(--surface); padding: 6px 12px;
        border-radius: 20px; border: 1px solid var(--border); font-size: 13px; font-weight: 600;
        margin-bottom: 15px; gap: 6px; backdrop-filter: blur(10px);
    }
    .pulse-dot {
    ...
    .card-title { 
        font-weight: 700; color: var(--foreground); font-size: 1.1rem; margin-bottom: 0.75rem; line-height: 1.375; 
    ...
    /* --- THE MAGIC iOS SLIDING GLASS CSS FOR NATIVE STREAMLIT RADIO --- */
    /* 1. Target the main radio container to create the pill background */
    ...
    /* 2. Hide the ugly default radio circles entirely */
    div[data-testid="stRadio"] div[role="radiogroup"] label > div:first-child {
        display: none !important;
    }

    /* 3. Style the clickable labels as pill buttons */
    ...
    /* 4. The Active "Sliding Glass" State */
    div[data-testid="stRadio"] div[role="radiogroup"] label[data-checked="true"] {
        color: #F8FAFC !important;
        background-color: rgba(255, 255, 255, 0.15) !important;
        box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2), inset 0 0 0 1px rgba(255, 255, 255, 0.1) !important;
    }

    /* Hover effect for inactive pills */
    div[data-testid="stRadio"] div[role="radiogroup"] label[data-checked="false"]:hover {
    ...
    .filter-label-text {
        ...
        text-transform: uppercase; letter-spacing: 0.5px; margin-right: 10px;
        display: flex; align-items: center; height: 100%;
    }
    
    /* Aligning the label and the pills */
    .custom-filter-row {
...
if not df.empty:
    ...
    # --- NEW NATIVE STREAMLIT FILTERING (Zero Page Reloads) ---
    all_categories = sorted(list(df['category'].dropna().unique()))
    options = ["All"] + all_categories
    
    # Read from URL params if someone shares a link
    init_cat = st.query_params.get("category", "All")
    if init_cat not in options:
        init_cat = "All"
    default_idx = options.index(init_cat)

    col_filter, col_toggle = st.columns([10, 2])
    
    with col_filter:
        st.markdown('<div class="custom-filter-row"><span class="filter-label-text">Filter by:</span></div>', unsafe_allow_html=True)
        # Using Streamlit's native radio widget. The CSS above completely transforms how it looks!
    ...
    # Update URL query params smoothly without reloading
    st.query_params["category"] = selected_cat
    ...
    with tab1:
        ...
        for index, row in filtered_df.reset_index().iterrows():
            ...
            # Use the robust function to guarantee colors never drop out
            cat_val = row['category']
            cat_color, cat_bg = get_category_style(cat_val)
